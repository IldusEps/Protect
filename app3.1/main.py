...
class MainContainer(Screen):
	def StartStop(self,state):
		if state == 'down':
			App.store.put('use',uses='2',id_=App.store.get('use')['id_'],time=App.store.get('use')['time'])
			
			activity.restartNotify(self.drop.current_item,self.drop.text)
		else:
			activity.stopNotify()
			App.store.put('use',uses='1',id_=App.store.get('use')['id_'],time=App.store.get('use')['time'])
	def Start(self):
		App.store.put('use',uses='0',id_='0',time='5sec.')
	def on_enter(self):
		Clock.schedule_once(self.init_,2.0)
	def setting(self, instance):
		self.drop.set_item(instance.text)
		print('Hellooo')
		App.store.put('use',uses=App.store.get('use')['uses'],id_=App.store.get('use')['id_'],time=instance.text)
		self.menu.dismiss()
	def init_(self,dt):
		menu_items = [{"text": "5sec."},{"text": "10sec."},{"text": "15sec."},{"text": "30sec."},{"text": "1min."},{"text": "3min."}]
		...
		if (App.store.exists('use')==True):
			self.drop.set_item(App.store.get('use')['time'])
...
class firstContainer(Screen):
	...
	def enter(self):
		...
		if g<1:
			g=1
		App.store.put('use',id_=str(g),uses=App.store.get('use')['uses'],time=App.store.get('use')['time'])
...
class sm(ScreenManager):
	def Enter_(self):
		...
		if (App.store.exists('use')==False):
			App.store.put('use',uses='0',id_='0',time=App.store.get('use')['time'])
			print('Hi')
	...
	def Form2(self):
		...
		if (App.store.exists(lang)==True):
			for it in icons_item:
				icons_item[it]=App.store.get(lang)['main'+it]
		print(icons_item["0"])
		self.current_screen.nav_layout.ids.content_drawer.ids.md_list.clear_widgets()
